...
with open(sys.argv[1], 'r') as r:
    ...
    for i in range(0, size):
        ...
        if isDuotone:
            filename = filename[:-8]
        print("glyph %d is %s" % (i, glyphs[i]))
        with open(filename + ".svg", 'w') as w:
            w.write('<?xml version="1.0" standalone="no"?>\n')
            w.write(
                '<svg width="1500" height="1500" viewBox="0 0 1500 1500" version="1.1" xmlns="http://www.w3.org/2000/svg">\n')
            w.write(glyphs[i].replace(
                '<glyph', '<path style="fill: #8E44AD" transform="scale(1, -1) translate(0, -1500)"') + '\n')
            if (isDuotone):
                w.write(glyphs[i+size].replace(
                    '<glyph', '<path style="fill: #3498DB" transform="scale(1, -1) translate(0, -1500)"') + '\n')
            w.write('</svg>')
